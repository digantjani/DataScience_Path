...
## Compactly displaying contests of list.
str(str)
str(lm)
...
## DNORM = evaluate the Normal probability density(with given mean or S/D) at a point (or vacotr of points)
## PNORM = evaluate the cumulative distribution function for a normal distribution
## RPOIS = generate random Poisson variates with a given rate.
## QPOIS = evaluate the inverse cumulative distribution function for the Poisson distribution

## d for density
## r for random number
## p for Cumulative Disturbution
## q for quantile function.

## Working with normal distributions we need following functions
  dnorm(x, mean = 0, sd = 1,log = FALSE)
  pnorm(q, mean = 0, sd = 1, lower.tail= TRUE, log.p = FALSE)
  qnorm(q, mean = 0, sd = 1, lower.tail= TRUE, log.p = FALSE)
  rnorm(n, mean = 0, sd = 1)

## Genrating Random Numbers from a Genralized Linerar model
  ## Y ~ POISSON(U)
  ## log U = Bo + b1x
  ## B0 = 5 and B1 = 0.3
...
## SAMPLE FUNCTION
  ...
  sample(1:10, replace=TRUE) ## Sample w/replacement  Numbers can repeat

## PROFILING
  ## Profiling is systemic way to examine how much time is spend in different parts of a program.
  ## useful in optimizing the code.
  ## SYSTEM.TIME() - Return in proc_time
    ## USER TIME: Time charged by CPU
    ## Elapsed time: Wall clock time
    ## They are close usually.
    ## If ET > UT then CPU spend lot of time on waiting around.
    ## If ET < UT then Multiple cores usage. Multithreaded BLAS libraries (MKL, ATLAS,ACML etc) also PRarellel processing.
    ## ET > UT Example
    system.time(readLines("https://www.asurams.edu"))
    ## ET < UT example
    hibert <- function(n) {
        i <-1:n
        1/outer(i-1, i, "+")
